...
func makeContactFormHandlerTailwind() func(*gin.Context) {
	return func(c *gin.Context) {
		c.Request.ParseForm()
		email := c.Request.FormValue("email")
		name := c.Request.FormValue("name")
		...
		_, err := mail.ParseAddress(email)
		if err != nil {
			TemplRender(c, http.StatusOK, tailwind.MakeContactFailure(email, err.Error()))
			return
		}

		// Make sure name and message is reasonable
		if len(name) > 200 {
			TemplRender(c, http.StatusOK, tailwind.MakeContactFailure(email, "name too long (200 char max)"))
			return
		}

		if len(message) > 10000 {
			TemplRender(c, http.StatusOK, tailwind.MakeContactFailure(email, "message too long (10000 char max)"))

			return
		}

		TemplRender(c, http.StatusOK, tailwind.MakeContactSuccess(email, name))
...
func contactHandlerTailwind(c *gin.Context, db *database.Database) ([]byte, error) {
